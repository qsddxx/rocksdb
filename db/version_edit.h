...
struct Segment {
  ...
  explicit Segment(std::vector<std::vector<FileMetaData*>>& filelist,
                   const InternalKeyComparator* cmp, uint64_t segment_id)
      : files_(std::move(filelist)), segment_id_(segment_id) {
    if (files_.empty() || files_[0].empty()) {
      return;
    }
    smallest = files_[0][0]->smallest;
  ...
  int GetLevelNum() { return files_.size(); }
  bool IsEmpty() { return file_num_ <= 0; }
